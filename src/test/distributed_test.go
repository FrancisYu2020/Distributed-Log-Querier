...
func TestDistributedServerEmpty(t *testing.T) {
	// Test query log from server on distributed machines
	// Check on empty log files. 
...
func TestDistributedServerMedium(t *testing.T) {
	// Test query log from server on distributed machines
	// Check on medium size log files. 
...
func TestDistributedServerLowFreq(t *testing.T) {
	// Test query log from server on distributed machines
	// Check on low frequency pattern log. 
	...
	// execute the grep command from the client
	start := time.Now()
	cmd := exec.Command(BIN_PATH_CLIENT, "grep", "-c", QUERY_EXPRESSION, "low_freq.log")
	elapsed := time.Since(start)
	log.Printf("%s execution time is: %s", cmd, elapsed)
	stdoutStderr, err = cmd.CombinedOutput()
	if err != nil {
	...
	// check if the total number of word count is correct
	second_last_line := log_lines[len(log_lines) - 3]
	if second_last_line[len(second_last_line) - 3:] != "400" {
		t.Errorf("Number of total word count does not match! Expected 1200000, got %s", second_last_line[len(second_last_line) - 3:])
		return
	}
	...
	var pass int
	for _, line := range log_lines {
		if len(line) <= 6 || line[len(line) - 3:] != "100" { continue }
		if line[3:5] == "01" || line[3:5] == "02" || line[3:5] == "03" || line[3:5] == "04" {
			pass ++
		} 
	}
...
func TestDistributedServerMediumFreq(t *testing.T) {
	// Test query log from server on distributed machines
	// Check on low frequency pattern log. 
	...
	// execute the grep command from the client
	start := time.Now()
	cmd := exec.Command(BIN_PATH_CLIENT, "grep", "-c", QUERY_EXPRESSION, "medium_freq.log")
	elapsed := time.Since(start)
	log.Printf("%s execution time is: %s", cmd, elapsed)
	stdoutStderr, err = cmd.CombinedOutput()
	if err != nil {
	...
	// check if the total number of word count is correct
	second_last_line := log_lines[len(log_lines) - 3]
	if second_last_line[len(second_last_line) - 5:] != "40000" {
		t.Errorf("Number of total word count does not match! Expected 40000, got %s", second_last_line[len(second_last_line) - 5:])
		return
	}
	...
	var pass int
	for _, line := range log_lines {
		if len(line) <= 6 || line[len(line) - 5:] != "10000" { continue }
		if line[3:5] == "01" || line[3:5] == "02" || line[3:5] == "03" || line[3:5] == "04" {
			pass ++
		} 
	}
...
func TestDistributedServerHighFreq(t *testing.T) {
	// Test query log from server on distributed machines
	// Check on high frequency pattern log. 
	...
	// execute the grep command from the client
	start := time.Now()
	cmd := exec.Command(BIN_PATH_CLIENT, "grep", "-c", QUERY_EXPRESSION, "high_freq.log")
	elapsed := time.Since(start)
	log.Printf("%s execution time is: %s", cmd, elapsed)
	stdoutStderr, err = cmd.CombinedOutput()
	if err != nil {
	...
	// check if the total number of word count is correct
	second_last_line := log_lines[len(log_lines) - 3]
	if second_last_line[len(second_last_line) - 8:] != "40000000" {
		t.Errorf("Number of total word count does not match! Expected 40000000, got %s", second_last_line[len(second_last_line) - 8:])
		return
	}
	...
	var pass int
	for _, line := range log_lines {
		if len(line) <= 6 || line[len(line) - 8:] != "10000000" { continue }
		if line[3:5] == "01" || line[3:5] == "02" || line[3:5] == "03" || line[3:5] == "04" {
			pass ++
		} 
	}
